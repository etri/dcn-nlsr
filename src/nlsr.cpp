...
Nlsr::Nlsr(ndn::Face& face, ndn::KeyChain& keyChain, ConfParameter& confParam)
  ...
  , m_fib(m_face, m_scheduler, m_adjacencyList, m_confParam, keyChain) // only one
  , m_routingTable(m_scheduler, m_fib, m_lsdb, m_namePrefixTable, m_confParam) // multi
  , m_namePrefixTable(m_fib, m_routingTable, m_routingTable.afterRoutingChange) // only one
  , m_lsdb(m_face, keyChain, m_confParam, m_namePrefixTable, m_routingTable, m_controller) // multi
  , m_helloProtocol(m_face, keyChain, confParam, m_routingTable, m_lsdb)
  , m_onNewLsaConnection(m_lsdb.getSync().onNewLsa->connect(  // only one
  ...
  , m_onHelloDataValidated(m_helloProtocol.onHelloDataValidated.connect(
      [this] (const ndn::Name& neighbor) { 
		...
		ndn::Name syncName("localhop");	
			syncName.append(m_confParam.getNetwork());
			syncName.append("nlsr");
			std::string area="area-";
			area += std::to_string(it->getAreaId());
			syncName.append(area);
			syncName.append("sync");
			syncName.appendVersion(m_confParam.getVersion());
          m_fib.registerPrefix(syncName, it->getFaceUri(), it->getLinkCost(),
  ...
  , m_prefixUpdateProcessor(m_dispatcher,
		  ...
		  m_namePrefixList,
		  m_lsdb,
		  m_confParam.getConfFileNameDynamic(), m_confParam.m_npl2)
  , m_nfdRibCommandProcessor(m_dispatcher,
		  m_namePrefixList,
		  m_lsdb, m_confParam.m_npl2)
  , m_statsCollector(m_lsdb, m_helloProtocol)
  , m_faceMonitor(m_face)
  , m_nlsrSignalSet(m_face.getIoService())
{
  NLSR_LOG_DEBUG("Initializing Nlsr");

  m_nlsrSignalSet.add(SIGINT);
  m_nlsrSignalSet.add(SIGTERM);

  m_nlsrSignalSet.async_wait(bind(&Nlsr::terminate, this, _1, _2));
 ...
 void Nlsr::terminate(const boost::system::error_code& error, int signalNo)
 {
     std::cout << "Nlsr::terminating..." << std::endl;
...
void Nlsr::setMonitoringInterestFilter()
{
    ndn::Name name = "/localhost/nlsr/monitoring";

    NLSR_LOG_DEBUG("Setting interest filter for MonitoringPrefix: " << name);

    m_face.setInterestFilter(ndn::InterestFilter(name).allowLoopback(false),
            std::bind(&Lsdb::processMonitoringInterest, &m_lsdb, _1, _2),
...
Nlsr::setLsaInterestFilter()
{
  ...
  for (auto&& areaId : m_confParam.getAreaIdList()) {
       ...
       ndn::Name lsaNamePrefix("localhop");
       lsaNamePrefix.append(m_confParam.getNetwork());
       lsaNamePrefix.append("nlsr");
       lsaNamePrefix.append("area-"+std::to_string(areaId));
       lsaNamePrefix.append("LSA");

	  m_face.setInterestFilter(ndn::InterestFilter(lsaNamePrefix).allowLoopback(false),
...
#if 1
	for (auto&& areaId : m_confParam.getAreaIdList()) {
		ndn::Name lsaName("localhop");
		lsaName.append(m_confParam.getNetwork());
		lsaName.append("nlsr");
		lsaName.append("area-"+std::to_string(areaId));
		lsaName.append("LSA");
		m_fib.setStrategy(lsaName, Fib::MULTICAST_STRATEGY, 0);

		ndn::Name syncName("localhop");
		syncName.append(m_confParam.getNetwork());
		syncName.append("nlsr");
		syncName.append("area-"+std::to_string(areaId));
		syncName.append("sync");
		m_fib.setStrategy(syncName, Fib::MULTICAST_STRATEGY, 0);
	}
#else
...
Nlsr::initialize()
{
  ...
  NLSR_LOG_DEBUG(m_namePrefixList);
    ...
    for (auto&& area : m_confParam.getAreaIdList()) {
		m_lsdb.buildAndInstallOwnNameLsa( area );
    }
#else
    ...
    case ndn::nfd::FACE_EVENT_DESTROYED: {
      ...
      if (adjacent != m_adjacencyList.end()) {
        ...
        if (adjacent->getStatus() == Adjacent::STATUS_ACTIVE) {
          ...
          if (m_confParam.getHyperbolicState() == HYPERBOLIC_STATE_ON) {
            m_routingTable.scheduleRoutingTableCalculation(adjacent->getAreaId());
          }
          else {
            // Will call scheduleRoutingTableCalculation internally
            // if needed in case of LS or DRY_RUN
            m_lsdb.scheduleAdjLsaBuild(adjacent->getAreaId());
          }
        }
      }
      break;
...
Nlsr::processFaceDataset(const std::vector<ndn::nfd::FaceStatus>& faces)
{
  ...
  for (auto&& adjacent : m_adjacencyList.getAdjList()) {
	  ...
	  // modified by ETRI(modori) on 20210603
	    //const std::string& faceUriString = adjacent.getFaceUri().toString();
	    const std::string& faceUriString = adjacent.getFaceUri().getHost();
    ...
    for (const auto& faceStatus : faces) {
	  ...
	  // modified by ETRI(modori) on 20210603
	  ndn::FaceUri uri(faceStatus.getRemoteUri());
	NLSR_LOG_DEBUG("RemoteUri: " << faceStatus.getRemoteUri() << ", " << faceUriString );
	  //if (adjacent.getFaceId() == 0 && faceUriString == faceStatus.getRemoteUri()) {
	  if (adjacent.getFaceId() == 0 && adjacent.getFaceUri().getHost() == uri.getHost()
			  and adjacent.getFaceUri().getScheme() == uri.getScheme()
		 ){
                                ...
                                const ndn::time::milliseconds& timeout)
{
    ...
    auto areaIt = m_confParam.getAreaIdList().find(adj.getAreaId());
    if( areaIt == m_confParam.getAreaIdList().end() )
        return ;

    ndn::Name lsaName("localhop");  
    lsaName.append(m_confParam.getNetwork());
    lsaName.append("nlsr");
    lsaName.append("area-"+std::to_string(*areaIt));
    lsaName.append("LSA");
    m_fib.registerPrefix(lsaName,
            faceUri, linkCost, timeout, ndn::nfd::ROUTE_FLAG_CAPTURE, 0);

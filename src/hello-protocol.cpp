  ...
  , m_adjacencyList(m_confParam.getAdjacencyList())
{
  ...
  for( auto &adj: m_confParam.getAdjacencyList() ){
	  //std::cout << "adJ's Area: " << adj.getAreaId() << std::endl;

		ndn::Name name(m_confParam.getRouterPrefix());
	  std::string area = "area-";
	  area += std::to_string(adj.getAreaId());
...
HelloProtocol::sendHelloInterest(const ndn::Name& neighbor)
{
  ...
  if(adjacent->getFaceId() != 0) {
    // interest name: /<neighbor>/NLSR/area#/INFO/<router>
    ndn::Name interestName = adjacent->getName() ;
// added by ETRI(modori) on 20201118
      std::string areaCom = "area-";
      areaCom += std::to_string(adjacent->getAreaId());

    interestName.append(NLSR_COMPONENT);
      interestName.append(areaCom);
...
// added by ETRI(modor) on 20201118
std::string areaCom = "area-";
areaCom += std::to_string(adjacent->getAreaId());
...
//ETRI
  ...
  ndn::Name neighbor = interestName.getPrefix(-4);
  NLSR_LOG_DEBUG("Neighbor: " << neighbor);
...
{
  // data name: /<neighbor>/NLSR/area#/INFO/<router>/<version>
...
//ETRI
    ...
    ndn::Name neighbor = dataName.getPrefix(-5);
  auto adjacent = m_adjacencyList.findAdjacent(neighbor);

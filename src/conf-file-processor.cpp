  ...
  for (const auto& tn : section) {
    if (tn.first == "neighbor") {
      try {
        ...
        uint64_t areaId = CommandAttriTree.get<uint64_t>("area-id", 0);
        ndn::Name neighborName(name);
        if (!neighborName.empty()) {
          Adjacent adj(name, faceUri, linkCost, Adjacent::STATUS_INACTIVE, 0, 0, areaId);
          m_confParam.getAdjacencyList().insert(adj);
          m_confParam.getAreaIdList().insert(areaId);
        }
        else {
...
ConfFileProcessor::processConfSectionAdvertising(const ConfigSection& section)
{
	...
	for(;it!=section.end();it++){
		...
		for (const auto& t : tn) {
			if(t.first == "id"){
				areaId = t.second.data(); 
			}else if(t.first == "prefix"){
				try {
					ndn::Name namePrefix(t.second.data());
					if (!namePrefix.empty()) {
						m_confParam.getNamePrefixList(stoi(areaId)).insert(namePrefix, areaId);
					}   
					else {

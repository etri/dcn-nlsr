...
public:
  ...
  using IsLsaNew =
    std::function<bool(const ndn::Name&, const Lsa::Type& lsaType, const uint64_t&, const uint64_t&)>;
   ...
   */
  void
  publishRoutingUpdate(uint64_t area, const Lsa::Type& type, const uint64_t& seqNo);

  //modori on 20210614
  void
  addUserNode(uint64_t toArea, const Lsa::Type& type);
...
PUBLIC_WITH_TESTS_ELSE_PRIVATE:
  ...
  processUpdateFromSync(const ndn::Name& originRouter,
                        const ndn::Name& updateName, uint64_t seqNo, uint64_t);
...
PUBLIC_WITH_TESTS_ELSE_PRIVATE:
  ...
  ndn::Name m_interNameLsaUserPrefix;
  ndn::Name m_externalNameLsaUserPrefix;

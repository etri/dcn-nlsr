...
PrefixUpdateProcessor::PrefixUpdateProcessor(ndn::mgmt::Dispatcher& dispatcher,
  ...
  : CommandManagerBase(dispatcher, namePrefixList, lsdb, "prefix-update", npl)
  ...
  , m_confFileNameDynamic(configFileName)
{
  ...
  m_dispatcher.addControlCommand<ndn::nfd::ControlParameters>(makeRelPrefix("redistribute"),
    makeAuthorization(),
    std::bind(&PrefixUpdateProcessor::validateParameters<RedistributePrefixCommand>,
                this, _1),
    std::bind(&PrefixUpdateProcessor::redistributePrefix, this, _1, _2, _3, _4));
}
...
bool
PrefixUpdateProcessor::addOrDeletePrefix(const ndn::Name& prefix, bool addPrefix, uint64_t areaId)
{
    std::cout << "addOrDeletePrefix " << prefix << ", areaId: " << areaId << std::endl;
...
ndn::optional<bool>
PrefixUpdateProcessor::afterRedistribute(const ndn::Name& prefix)
{
  return addOrDeletePrefix(prefix, true, 777);
}
ndn::optional<bool>
PrefixUpdateProcessor::afterAdvertise(const ndn::Name& prefix, uint64_t areaId)
{
  return addOrDeletePrefix(prefix, true, areaId);
}

ndn::optional<bool>
PrefixUpdateProcessor::afterWithdraw(const ndn::Name& prefix, uint64_t areaId)
{
  return addOrDeletePrefix(prefix, false, areaId);
}

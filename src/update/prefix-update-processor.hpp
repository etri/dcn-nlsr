...
public:
   ...
   */
  bool
  addOrDeletePrefix(const ndn::Name& prefix, bool addPrefix,uint64_t);

  ndn::optional<bool>
  afterRedistribute(const ndn::Name& prefix);

  ndn::optional<bool>
  afterAdvertise(const ndn::Name& prefix, uint64_t);

  ndn::optional<bool>
  afterWithdraw(const ndn::Name& prefix, uint64_t);

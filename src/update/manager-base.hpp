...
public:
  CommandManagerBase(ndn::mgmt::Dispatcher& m_dispatcher,
		     ...
		     std::map<uint64_t, std::shared_ptr<NamePrefixList>> &
);
  ...
  void
  redistributePrefix(const ndn::Name& prefix,
                          const ndn::Interest& interest,
                          const ndn::mgmt::ControlParameters& parameters,
                          const ndn::mgmt::CommandContinuation& done);
  ...
  afterAdvertise(const ndn::Name& prefix, uint64_t) { return ndn::nullopt; }

  /*! \brief save an advertised prefix to the nlsr configuration file
   *         returns bool from the overridden function while nullopt here
   */
  virtual ndn::optional<bool>
  afterWithdraw(const ndn::Name& prefix, uint64_t) { return ndn::nullopt; }

protected:
  NamePrefixList& m_namePrefixList;
  std::map<uint64_t, std::shared_ptr<NamePrefixList>>& m_namePrefixList2;
  Lsdb& m_lsdb;
};

...
public:
  ...
  bool
  isABR(){
	return m_areaIdList.size() > 1 ? true:false;
}
  ...
  NamePrefixList&
  getNamePrefixList(uint64_t area)
  {
	  if( m_npl2.find(area) == m_npl2.end() ){
		auto npl = std::make_shared<NamePrefixList>();
		m_npl2[area] = npl;
	  }
    return *m_npl2[area];
  }
  ...
  uint64_t getVersion() { return SYNC_VERSION;}
  std::map<uint64_t, std::shared_ptr<NamePrefixList> >m_npl2;

  ...
  , m_routingTable(routingTable)
{
  m_afterRoutingChangeConnection = afterRoutingChangeSignal->connect(
    [this] (const std::list<RoutingTableEntry>& entries, uint64_t& areaId) {
      updateWithNewRoute(entries, areaId);
    });
}
...
void
NamePrefixTable::addEntry(const ndn::Name& name, const ndn::Name& destRouter, Lsa::Type lsaType, uint64_t areaId)
{
  ...
  NptEntryList::iterator nameItr =
    std::find_if(m_table.begin(), m_table.end(),
                 [&] (const std::shared_ptr<NamePrefixTableEntry>& entry) {
                     // 이름으로만 확인 후, LsaType의 우선순위에 따라서 삭제여부 판단
  ...
  if (nameItr != m_table.end()) {
    npte = *nameItr;
    if(npte->getLsaType() < lsaType){
        removeEntry(name, destRouter, areaId);
        nameItr = m_table.end();
    }
  }
#endif
...
//modori on 20210616
  auto rtpoolIt = m_rtpool2.find(areaId);
  if(rtpoolIt == m_rtpool2.end()){
    m_rtpool2[areaId] = std::make_shared<RoutingTableEntryPool>();
  }
  auto rtpool = m_rtpool2[areaId];

  // Attempt to find a routing table pool entry (RTPE) we can use.
  RoutingTableEntryPool::iterator rtpeItr = rtpool->find(destRouter);
  ...
  if (rtpeItr == rtpool->end()) {
  //if (rtpeItr == m_rtpool.end()) {
    // See if there is a routing table entry available we could use
    RoutingTableEntry* routeEntryPtr = m_routingTable.findRoutingTableEntry(destRouter, areaId);
    ...
    rtpePtr = addRtpeToPool(rtpe, areaId);
  }
  ...
  if (nameItr == m_table.end()) {
    ...
    npte = std::make_shared<NamePrefixTableEntry>(name, lsaType);
...
void
NamePrefixTable::removeEntry(const ndn::Name& name, const ndn::Name& destRouter, uint64_t areaId)
{
    ...
    auto rtpoolIt = m_rtpool2.find(areaId);
    if(rtpoolIt == m_rtpool2.end() ){
        std::cout << "There is no Entry to be remove on Area: " << areaId << std::endl;
        return;
    }

    auto rtpool = rtpoolIt->second;
    ...
    if (nameItr != m_table.end()) {
        ...
        if ((*nameItr)->removeRoutingTableEntry(rtpePtr) == 0) {
            deleteRtpeFromPool(rtpePtr, areaId);
        }
...
void
NamePrefixTable::updateWithNewRoute(const std::list<RoutingTableEntry>& entries, uint64_t&areaId)
{
  NLSR_LOG_DEBUG("Updating table with newly calculated routes on Area:" << areaId);

  //modori on 20210616
  auto rtpoolIt = m_rtpool2.find(areaId);
  if( rtpoolIt == m_rtpool2.end() ){
        std::cout << "There is no rtpool2's Entry with area: " << areaId << std::endl;
        return;
  }
  auto rtpool = rtpoolIt->second;

  // Iterate over each pool entry we have
  for (auto&& poolEntryPair : *rtpool) {
      //DOMODRI on 20210616
    ...
    if (sourceEntry != entries.end()
      ...
      for (const auto& nameEntry : poolEntry->namePrefixTableEntries) {
        ...
        addEntry(nameEntryFullPtr->getNamePrefix(), poolEntry->getDestination(), 
            nameEntryFullPtr->getLsaType(), areaId );
      }
    }
...
NamePrefixTable::addRtpeToPool(RoutingTablePoolEntry& rtpe, uint64_t areaId)
{
    auto rtpool = m_rtpool2[areaId];// = std::make_shared<RoutingTableEntryPool>();

  RoutingTableEntryPool::iterator poolItr =
  //modori on 20210616
    //m_rtpool.insert(std::make_pair(rtpe.getDestination(),
    rtpool->insert(std::make_pair(rtpe.getDestination(),
  ...
  // isn't in the pool.
void
NamePrefixTable::deleteRtpeFromPool(std::shared_ptr<RoutingTablePoolEntry> rtpePtr, uint64_t areaId)
{
    auto rtpool = m_rtpool2[areaId];
  if (rtpool->erase(rtpePtr->getDestination()) != 1) {

...
#endif
  const std::list<RoutingTableEntry>&
  getRoutingTableEntry(uint64_t area) const
  {
      auto it = m_rTable.find(area);
      if(it!=m_rTable.end())
        return *it->second;
  }
...
PUBLIC_WITH_TESTS_ELSE_PROTECTED:
  ...
  std::map<uint64_t, std::shared_ptr<std::list<RoutingTableEntry>>> m_rTable;
...
public:
   ...
   */
  void
  addNextHop(const ndn::Name& destRouter, NextHop& nh, uint64_t);
  ...
  RoutingTableEntry*
  findRoutingTableEntry(const ndn::Name& destRouter, uint64_t);

  /*! \brief Schedules a calculation event in the event scheduler only
   *  if one isn't already scheduled.
   */
  void
  scheduleRoutingTableCalculation(uint64_t);
...
private:
  ...
  void
  calculateLsRoutingTable(uint64_t);

                                               ...
                                               const Lsdb& lsdb, uint64_t area)
{
  ...
  if (sourceRouter && confParam.getMaxFacesPerPrefix() == 1) {
    ...
    addAllLsNextHopsToRoutingTable(confParam.getAdjacencyList(), rt, pMap, *sourceRouter, area);
  }
  else {
    // Multi Path
...
LinkStateRoutingTableCalculator::addAllLsNextHopsToRoutingTable(AdjacencyList& adjacencies,
                                                                ...
                                                                uint32_t sourceRouter, uint64_t area)
{
  NLSR_LOG_DEBUG("LinkStateRoutingTableCalculator::addAllNextHopsToRoutingTable Called on Area: " << area);
    ...
    if (i != sourceRouter) {
      ...
      if (nextHopRouter != NO_NEXT_HOP) {
        ...
        if (nextHopRouterName) {
          ...
          rt.addNextHop(*(pMap.getRouterNameByMappingNo(i)), nh, area);
...
#if 0
  auto srcLsa  = lsdb.findLsa<CoordinateLsa>(src, 777);
  auto destLsa = lsdb.findLsa<CoordinateLsa>(dest, 777);

...
ProducerBase::addUserNode(const ndn::Name& prefix)
{
  if (m_prefixes.find(prefix) == m_prefixes.end()) {
    m_prefixes[prefix] = 0;
/*    std::cout << "ProducerBase::addUserNode: " << prefix << std::endl;
    for (std::map<ndn::Name,uint64_t>::iterator it=m_prefixes.begin(); it!=m_prefixes.end(); ++it)
            std::cout << it->first.toUri() << " => " << it->second << '\n';
*/
    return true;
  }
  else {

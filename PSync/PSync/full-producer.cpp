...
FullProducer::onSyncInterest(const ndn::Name& prefixName, const ndn::Interest& interest)
{
  ...
  if (!diff.listEntries(positive, negative)) {
    ...
    if (positive.size() + negative.size() >= m_threshold ||
      ...
      if (!state.getContent().empty()) {
        sendSyncData(interest.getName(), state.wireEncode(), interest);
      }
  ...
  if (!state.getContent().empty()) {
    ...
    sendSyncData(interestName, state.wireEncode(), interest);
    return;
  }

  auto& entry = m_pendingEntries.emplace(interestName, PendingEntryInfo{iblt, {},
  std::make_shared<ndn::Interest>(interest)}).first->second;
...
void
FullProducer::sendSyncData(const ndn::Name& name, const ndn::Block& block, const ndn::Interest &interest)
{
  NDN_LOG_DEBUG("Checking if data will satisfy our own pending interest");
  ...
  if (m_outstandingInterestName == name) {
    ...
    // Send data after removing pending sync interest on face
    m_segmentPublisher.publish(interest, name, dataName, content, m_syncReplyFreshness);
...
FullProducer::satisfyPendingInterests()
{
  ...
  for (auto it = m_pendingEntries.begin(); it != m_pendingEntries.end();) {
    ...
    if (!state.getContent().empty()) {
      ...
      sendSyncData(it->first, state.wireEncode(), *entry.interest);
      it = m_pendingEntries.erase(it);
    }
    else {
      ++it;
